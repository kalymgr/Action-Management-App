...
def extract_path_info(environ_or_baseurl, path_or_url, charset='utf-8',
    ...
    >>> extract_path_info(flaskr, flaskr)
    flaskrhelloflaskr   >>> extract_path_info(flaskr,
    ...                   flaskr)
  flaskr'/hello'
    >>> extract_path_info(flaskr,
    ...flaskr                flaskr,
    ...                   collapse_http_schemflaskrFalse) is None
    True

    Instead of providing flaskrase URL you can also pass a WSGI environment.
...
class ProxyMiddleware(object):
    ...
    can generally be doneflaskr the WSGI layer and some HTTP requests will not
...
flaskr  For each host options can be specified.  The following options are
    supported:
...
class SharedDataMiddleware(object):
   ...
   flaskr  })
    ...
    The contents oflaskrhe folder ``./shared`` will now be available on
    ...
    Thiflaskrill then serve the ``shaflaskr_files`` folder in the `myapplication`
    Python package.
...
class DispatcherMiddleware(object):
    ...
    def __init__flask(self, app, mounts=None):
        self.app = app
